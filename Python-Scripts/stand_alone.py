...
DRONE_URI = "udp://192.168.1.214"
TARGET_HEIGHT = 0.7  # Target hover height in meters
...
TRIM_VY = -0.09  # Left/right trim correction
# Battery monitoring
...
VELOCITY_KD = 0.0
# Control limits
MAX_CORRECTION = 1  # Maximum control correction allowed
...
def setup_logging(cf, logger=None):
  ...
  try:
      toc = cf.log.toc.toc
      # Quick TOC summary for debugging missing variables.
      if logger:
          logger("TOC groups: " + ", ".join(sorted(toc.keys())))
      ...
      for var_name, var_type in motion_variables:
          ...
          if group in toc and name in toc[group]:
              ...
              if logger:
                  available = ", ".join(sorted(toc.get(group, {}).keys())) if group in toc else "N/A"
                  logger(f"Motion variable not found: {var_name}. Available in '{group}': {available}")
      if len(added_motion_vars) < 2:
          if logger:
              logger("ERROR: Not enough motion variables found! Optical flow data will be unavailable.")
          return None, None
      ...
      for var_name, var_type in battery_variables:
          ...
          if group in toc and name in toc[group]:
              ...
              if logger:
                  available = ", ".join(sorted(toc.get(group, {}).keys())) if group in toc else "N/A"
                  logger(f"Battery variable not found: {var_name}. Available in '{group}': {available}")
      # Setup callbacks
...
class PositionHoldGUI:
  ...
  def create_ui(self):
      ...
      # IP address field
      ip_frame = tk.Frame(params_frame)
      ip_frame.pack(fill=tk.X, pady=2)
      tk.Label(ip_frame,text="IP address:",width=16,anchor=tk.W).pack(side=tk.LEFT)
      self.ip_entry_var = tk.StringVar(value=DRONE_URI)
      tk.Entry(ip_frame,textvariable=self.ip_entry_var,width=20).pack(side=tk.LEFT, padx=5)
  ...
  def apply_values(self):
      ...
      try:
          DRONE_URI = self.ip_entry_var.get().strip()
  ...
  def sensor_test_thread_func(self):
      ...
      try:
          ...
          uri = self.ip_entry_var.get().strip()
